...
let partTwo () =
    ...
    let busesWithoutFirstBus = buses |> Array.skip(1)
    ...
    let getTimestampIterator (bus: int64 * int64) =
        ...
        printfn "Iterator for Bus %d:\t%d + %di" busId ((indexInDiffArray * busId) - busIndex) (diffArrayLength * busId)
    ...
    let getTimestampChecker (bus: int64 * int64) =
        ...
        let indexInDiffArray = findInDifferencesArray (differencesArray, busIndex)
        let diffArrayLength = differencesArray |> Array.length |> int64
        let checker (timestamp: int64) =
            ((timestamp - ((indexInDiffArray * busId) - busIndex)) % (diffArrayLength * busId)) = 0L
        checker
        // ((indexInDiffArray * busId) - busIndex) + (diffArrayLength * busId)x = timestamp
        // a + bx = timestamp
        // (timestamp - a) % b = 0 ?

    for bus in busesWithoutFirstBus do
        let (busId, busIndex) = bus
        let differencesArray = getDifferencesArray (firstBusId, busId)
        let indexInDiffArray = findInDifferencesArray (differencesArray, busIndex)
        let diffArrayLength = differencesArray |> Array.length |> int64
        // printfn "%d is at index %d" busIndex indexInDiffArray
        // printfn "%d + %dx = %dy + %d" (indexInDiffArray * busId) (diffArrayLength * busId) firstBusId busIndex
        printfn "%d + %dx = %dy" ((indexInDiffArray * busId) - busIndex) (diffArrayLength * busId) firstBusId
        // printfn ""
    ...
    let iteratorStart = (timestampStart - 4147L) / 17429L
